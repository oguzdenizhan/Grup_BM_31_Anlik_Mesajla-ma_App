...
namespace MsgAppClient
{
    public partial class Client : Form
    {
        ...
        private void Log(string msg = "") // eğer mesaj kısmı boş ise logu temizle 
        {
            if (!exit)
            {
            ...
            if (!exit)
            {
                connectButton.Invoke((MethodInvoker)delegate
                {
                    ...
                    if (status)
                    {
                        ...
                        Log(SystemMsg("Bağlantıyı Sağladın."));
                    }
                    else
                    {
                        ...
                        Log(SystemMsg("Bağlantıyı Kestin."));
                    }
                });
            }
        }
        ...
        private void ConnectButton_Click(object sender, EventArgs e)
        {
            ...
            else if (client == null || !client.IsAlive)
            {
                ...
                {
                    error = true;
                    Log(SystemMsg("Port numarası geçersiz!"));
                }
                else if (port < 0 || port > 65535)
                {
                    error = true;
                    Log(SystemMsg("Port numarası aralık dışı"));
                }
        ...
        private void SendTextBox_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                ...
                if (sendTextBox.Text.Length > 0)
                {
                    ...
                    sendTextBox.Clear();
                    Log(string.Format("{0} (Sen): {1}", obj.username, msg));
                    if (connected)
                    {
                        Send(msg);
                    }
